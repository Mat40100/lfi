...
- **Forum (Discourse, standalone)** — a private team forum at `forum.landes-insoumises.fr`, installed OUTSIDE this compose stack via Discourse's own launcher in `/var/discourse` (standalone container, Postgres+Redis embedded), joined to the `lfi_web` network so Caddy can proxy it (server-only `forum.` site block in the prod Caddyfile). Accounts are Discourse-native: `login_required` + `invite_only` (admins invite people from Discourse). The former Ghost-SSO bridge (Discourse-on-Ghost connector, Équipe tier gate, team invite console) was **removed on 2026-09-05** — do not reintroduce a Ghost↔forum link. Runbook: **`docs/forum-discourse.md`**.
