...
- **Host:** `37.59.103.153` (OVH, eu-west, Ubuntu cloud image), domain **`adour-en-commun.fr`** (+ `www`), DNS on OVH nameservers. Previous domains `landes-insoumises.fr` (until 2026-09) and `lol-reminder.fr` (until 2026-07-31) 301-redirect to it while their DNS still points at the box.
- **Domain cutover (2026-09) — PENDING as of 2026-09-23** (`adour-en-commun.fr` not registered yet; prod still answers on `landes-insoumises.fr`): `landes-insoumises.fr → adour-en-commun.fr` is driven by `scripts/domain-cutover-adour.sh` (staged as `~/domain-cutover-adour.sh` on the server; runbook in `docs/forum-discourse.md` → "Domain migration"). Prereqs the script checks: the new apex/`www`/`forum` A records → `37.59.103.153` (delete OVH's parking `A 213.186.33.5`), Mailjet SPF+DKIM on the new domain. Drop this PENDING note once it has run.
