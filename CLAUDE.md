...
- **Theme: manual copy of Source v1.6.1** in `data/ghost-content/themes/source` (server-only, replaces the default symlink to the bundled theme). Why: the Source 1.5.0 bundled with Ghost 5.130.6 hardcodes its UI strings in English (no `{{t}}`/locales, so the fr locale had no effect), and Source ≥1.7 needs the `{{social_accounts}}` helper that Ghost 5.x lacks (crashes with "Missing helper"). v1.6.1 = newest translated version compatible with Ghost 5. Rollback leftovers sit next to it (`source-symlink.bak`, `source-171.broken`). When Ghost is upgraded to 6.x, drop the copy and restore the symlink (the bundled theme will be translated).
